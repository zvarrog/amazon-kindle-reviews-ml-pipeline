...
def sentiment_udf(texts: pd.Series) -> pd.Series:
    """Вычисляет sentiment для каждого текста в серии."""
    return texts.fillna("").apply(calculate_sentiment)


def create_spark_session() -> SparkSession:
    """Создаёт и настраивает Spark сессию."""
    spark = (
    ...
    log.info(
        "Spark: driver=%s, cores=%s, partitions=%s",
        SPARK_DRIVER_MEMORY,
        SPARK_NUM_CORES,
        SHUFFLE_PARTITIONS,
    )
...
def load_data(spark: SparkSession) -> DataFrame:
    """Загружает CSV данные в Spark DataFrame."""
...
def clean_and_balance_data(df: DataFrame) -> DataFrame:
    """Очищает данные и балансирует классы."""
...
def add_sentiment_features(df: DataFrame) -> DataFrame:
    """Добавляет признаки тональности в DataFrame."""
    return (
        ...
        .withColumn(
            "sentiment_strength",
            when(spark_abs(col("sentiment")) > 0.5, "strong")
            .when(spark_abs(col("sentiment")) > 0.2, "moderate")
            .otherwise("weak"),
        )
    )
...
def apply_tfidf(
    ...
    """Применяет TF-IDF преобразование к train/val/test."""
...
def add_aggregations(
    ...
    """Добавляет агрегированные признаки по пользователям и товарам."""
    user_stats = (
...
def process_data(force: bool = False) -> None:
    ...
    try:
        df = load_data(spark)
        clean = add_sentiment_features(clean_and_balance_data(df))
