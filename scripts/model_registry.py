...
class DistilBertWrapper(mlflow.pyfunc.PythonModel):
    """Обёртка для DistilBERT-модели для сохранения в MLflow."""

    def load_context(self, context: mlflow.pyfunc.PythonModelContext) -> None:
        """Загружает модель из артефактов."""
        self.model = joblib.load(context.artifacts["model_path"])

    def predict(self, _context: Any, model_input: Any) -> Any:
...
def should_replace_model(new_metric: float, old_metric: float | None, model_name: str) -> bool:
...
def _log_model_to_mlflow(model_path: Path, model_kind: ModelKind, model_name: str) -> None:
...
def _rotate_production_versions(client: MlflowClient, model_name: str, keep_n: int = 3) -> None:
