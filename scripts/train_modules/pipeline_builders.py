...
class DistilBertBuilder(PipelineBuilder):
    """Билдер для DistilBERT модели без числовых признаков."""

    def build(self) -> Pipeline:
        """Строит пайплайн с DistilBERT."""
...
class LogRegBuilder(PipelineBuilder):
    """Билдер для логрег с TF-IDF и числовыми признаками."""
    ...
    def build(self) -> Pipeline:
        ...
        penalty = (
            "l2"
            if solver == "lbfgs"
            else self.trial.suggest_categorical("logreg_penalty", ["l1", "l2"])
        )
...
class RandomForestBuilder(PipelineBuilder):
    """Билдер для случайного леса."""

    def build(self) -> Pipeline:
        """Строит пайплайн со случайным лесом."""
        use_stemming = self.trial.suggest_categorical("use_stemming", [False, True])
        text_max_features = self.trial.suggest_int(
            "tfidf_max_features",
            TFIDF_MAX_FEATURES_MIN,
...
class HistGBBuilder(PipelineBuilder):
    """Builder для HistGradientBoosting."""

    def build(self) -> Pipeline:
        """Строит Pipeline с HistGradientBoosting."""
        use_stemming = self.trial.suggest_categorical("use_stemming", [False, True])
        text_max_features = self.trial.suggest_int(
            "tfidf_max_features",
            TFIDF_MAX_FEATURES_MIN,
