...
class PredictProbaModel(Protocol):
    """Протокол для модели с predict_proba."""
    ...
    def classes_(self) -> np.ndarray:
        """Массив классов модели."""
        ...


def plot_roc_pr_curves(
    pipeline: PredictProbaModel,
    x_test: pd.DataFrame,
...
def _align_probability_shape(
    pipeline: PredictProbaModel, y_score: np.ndarray, y_true_bin: np.ndarray, classes: list[int]
) -> np.ndarray:
